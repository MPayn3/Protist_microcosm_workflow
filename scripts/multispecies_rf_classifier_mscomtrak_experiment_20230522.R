...
##                                                                    ##
##  Multi-species comtrak, multispecies classifier - Real Experiment  ##
##                                                                    ##
##                       May 22nd 2023                                ##
##                                                                    ##
########################################################################

## Testing secondary classifier on a real masters experiment from three species 
## P.caudatum, S.teres and S.virginianum
...
test_files_all <- list.files("data/natural_experiment_test/")[-9]
training_files_all <- list.files("data/natural_experiment_test/comtrak_train/")

## file names from track and class (TC)
test_files_tc <- test_files_all[grep("TC", test_files_all)]
training_files_tc <- training_files_all[grep("TC", training_files_all)]

files_tc_all <- c(test_files_tc, training_files_tc)

ms_mscomtrak_exp <- bind_rows(lapply(files_tc_all, function(x){
  
  ## Add specification for whether its a test/training file from comtrak
  if(x %in% test_files_tc == TRUE){
    c_type = "test"
    
    # raw data with species probabilities
    c_raw = read_delim(paste0("data/rawdata/natural_experiment_test/", x))
    
    # validated species names
    c_speciesnames = read_delim(paste0("data/rawdata/natural_experiment_test/", gsub("TC", "labels", x))) %>% 
    ...
    # raw data with species probabilities
    c_raw = read_delim(paste0("data/rawdata/natural_experiment_test/comtrak_train/", x))
    
    # validated species names
    c_speciesnames = read_delim(paste0("data/rawdata/natural_experiment_test/comtrak_train/", gsub("TC", "labels", x))) %>% 
      dplyr::select(Frame, ID, Species = Label)
  }
...
species_pred_orig <- ms_mscomtrak_exp %>% 
  # filter(type == "test") %>% 
...
# using all test/training data

dat <- ms_mscomtrak_exp %>% 
...
ggsave(OOB_plot, 
       filename = "output/plots/multi_species/ms_comtrak_experiment/OOB_error_mscomtrak_20230418.jpeg",
...
flextable(test_error) %>% 
  ...
  save_as_image(path = "output/plots/multi_species/ms_comtrak_experiment/multispecies_training_test_error.png")
...
ggsave(var_imp_plot, 
       filename = "output/plots/multi_species/ms_comtrak_experiment/multispecies_variable_importance.jpeg",
