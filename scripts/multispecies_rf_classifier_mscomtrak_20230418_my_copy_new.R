...
ms_comtrak <- bind_rows(lapply(tc.files, function(x){
  ...
  c_raw <- c_raw %>% 
    group_by(Frame, ID) %>%  # group the data
  ...
  c_raw <- c_raw %>% ungroup() %>%  # Ungroup before regrouping for new variables
    
    # Add Replicate from the file name
    mutate(Replicate = gsub(".txt", "", unlist(strsplit(x, "_"))[8])) %>% 
    
    # Remove non-protist/mixed data for now
    ...
    # Rename columns to lowercase and select relevant ones
    rename_with(tolower) %>% 
    dplyr::select(id, frame, time_s, replicate, species,
                  length_um, width_um, 
